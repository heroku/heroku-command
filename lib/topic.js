...
class Topic extends mixins.mix(Object).with(color(), output()) {
  ...
  listCommandsHelp ({argv0}) {
    ...
    let commands = this.commands.filter(c => !c.hidden)
    if (commands.length === 0) return
    this.log(`${argv0} ${this.constructor.topic} commands: (${this.color.cmd(argv0 + ' help ' + this.constructor.topic + ':COMMAND')} for details)
`)
    for (let command of commands) command.usage = this.usage(command)
    let maxLength = max(commands, 'usage.length').usage.length + 1
    for (let command of commands) {
      this.log(` ${S(command.usage).padRight(maxLength)}${command.description ? ' # ' + command.description : ''}`)
    }
    this.log()
  }
  ...
  linewrap (length, s) {
    const linewrap = require('../lib/linewrap')
    return linewrap(length, screen.errtermwidth(), {
      skipScheme: 'ansi-color'
    })(s).trim()
  }
}
